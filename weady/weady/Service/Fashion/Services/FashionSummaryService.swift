...
final class FashionSummaryService: NetworkManager {
    
    typealias Endpoint = FashionSummaryEndpoints
    
    // MARK: - Provider
    let provider: MoyaProvider<FashionSummaryEndpoints>
    
    init(provider: MoyaProvider<FashionSummaryEndpoints>? = nil) {
        // Logger 플러그인 구성
        let plugins: [PluginType] = [
            NetworkLoggerPlugin(configuration: .init(logOptions: .verbose))
        ]
        self.provider = provider ?? MoyaProvider<FashionSummaryEndpoints>(plugins: plugins)
    }
